...
public class EncryptResponseBodyAdvice implements ResponseBodyAdvice<Object> {
    ...
    public EncryptResponseBodyAdvice(SecretKeyConfig secretKeyConfig){
    ...
    public boolean supports(MethodParameter returnType, Class<? extends HttpMessageConverter<?>> converterType) {
        ...
        encrypt = method.isAnnotationPresent(EncryptedParams.class) && secretKeyConfig.isOpen();
        return encrypt;
    }
    ...
    public Object beforeBodyWrite(Object body, MethodParameter returnType, MediaType selectedContentType,
        ...
        if (encrypt) {
            ...
            try {
                String content = JsonUtil.writeValueAsString(body);
                if (!StringUtils.hasText(publicKey)) {
                    throw new NullPointerException("Please configure rsa.encrypt.privatekeyc parameter!");
                }
                byte[] data = content.getBytes();
                byte[] encodedData = {}; // TODO RSAUtil.encrypt(data, publicKey);
